...
public class Surforge : MonoBehaviour {  
	...
	// 静态成员变量
	public static SurforgeSettings surforgeSettings;  													// 静态成员变量
	static List<SurforgeVoxelizedSet> voxelizedSets = new List<SurforgeVoxelizedSet>();					// 体素化集合列表
	static List<SurforgeSceneCluster> sceneClusters = new List<SurforgeSceneCluster>();					// 场景聚类列表
    ...
    static void GetSettings(bool isMetallic) {
        ...
        SurforgeSettings es = (SurforgeSettings)GameObject.FindFirstObjectByType(typeof(SurforgeSettings));
#else
		...
		if (es == null) {
			...
			FillTexture(surforgeSettings.objectMasks, new Color(0,0,0,1)); 
			FillTexture(surforgeSettings.objectMasks2, new Color(0,0,0,0)); 
			FillTexture(surforgeSettings.normalMap, new Color(0.5f,0.5f,1,1));
			FillTexture(surforgeSettings.aoEdgesDirtDepth, new Color(1, 0, 1, 1));

			FillTexture(surforgeSettings.emissionMap, new Color(0,0,0,1));
			FillTexture(surforgeSettings.labelsMap, new Color(0,0,0,0));
			FillTexture(surforgeSettings.labelsAlpha, new Color(0,0,0,0));
		}
		else {
			surforgeSettings = es;
	...
	*/
			...
			if (surforgeSettings.placeMeshes.placeMeshes[surforgeSettings.activePlaceMesh].shuffleArray != null) {
				if (surforgeSettings.placeMeshes.placeMeshes[surforgeSettings.activePlaceMesh].shuffleArray.Length > 0) {
					...
					if (surforgeSettings.placeToolPreview != null) {
                        ...
                        //shuffle meshes
                        if (surforgeSettings.placeMeshes.placeMeshes[surforgeSettings.activePlaceMesh].shuffleMeshes != null) {
                            if (surforgeSettings.placeMeshes.placeMeshes[surforgeSettings.activePlaceMesh].shuffleMeshes.Length > 0) {
                                int meshShuffleArrayLength = surforgeSettings.placeMeshes.placeMeshes[surforgeSettings.activePlaceMesh].shuffleMeshes.Length;
                                int meshShuffleRandom = Random.Range(0, meshShuffleArrayLength);
                                MeshFilter chosenMeshFilter = (MeshFilter)chosenPlaceMesh.GetComponent<MeshFilter>();
                                if (chosenMeshFilter != null) {
                                    Mesh chosenMesh = surforgeSettings.placeMeshes.placeMeshes[surforgeSettings.activePlaceMesh].shuffleMeshes[meshShuffleRandom];
                                    if (chosenMesh != null) {
                                        chosenMeshFilter.sharedMesh = chosenMesh;
                                    }
                                }
                            }
                        }
	...
	static void CreateTextures(int res) {
		...
		//render result
		if (surforgeSettings.normalMap.width != res)			surforgeSettings.normalMap.Reinitialize(res, res);
		if (surforgeSettings.emissionMap.width != res)			surforgeSettings.emissionMap.Reinitialize(res, res);
		if (surforgeSettings.objectMasks.width != res)			surforgeSettings.objectMasks.Reinitialize(res, res);
		if (surforgeSettings.objectMasks2.width != res)			surforgeSettings.objectMasks2.Reinitialize(res, res);
		if (surforgeSettings.aoEdgesDirtDepth.width != res)		surforgeSettings.aoEdgesDirtDepth.Reinitialize(res, res);
		if (surforgeSettings.labelsMap.width != res)			surforgeSettings.labelsMap.Reinitialize(res, res);
		if (surforgeSettings.labelsAlpha.width != res)			surforgeSettings.labelsAlpha.Reinitialize(res, res);
		...
		if (surforgeSettings.bucketTexture.width != res)		surforgeSettings.bucketTexture.Reinitialize(res, res); 
    ...
    static ShadowResolution initialShadowResolution;
    static ShadowProjection initialShadowProjection;
    static float initialShadowDistance;
    static float initialShadowPlaneOffset;
    static int initialShadowCascades;
    static float initialShadowCascade2Split;
    static Vector3 initialShadowCascade4Split;

    static void GetProjectShadowSettings() {
        initialShadowResolution = QualitySettings.shadowResolution;
        initialShadowProjection = QualitySettings.shadowProjection;
        initialShadowDistance = QualitySettings.shadowDistance;
        initialShadowPlaneOffset = QualitySettings.shadowNearPlaneOffset;
        initialShadowCascades = QualitySettings.shadowCascades;
        initialShadowCascade2Split = QualitySettings.shadowCascade2Split;
        initialShadowCascade4Split = QualitySettings.shadowCascade4Split;
    }

    static void SetProjectSettingsForRender(int aoMode) {
        QualitySettings.shadowDistance = 10000.0f;
        QualitySettings.shadowCascades = 4;
    ...
    public static void StartGPURender() {
        ...
        GetProjectShadowSettings();
        SetProjectSettingsForRender(surforgeSettings.GetAoMode());
        ...
        gpuRender.StoreProjectShadowSettings(initialShadowResolution,
                                             initialShadowProjection,
                                             initialShadowDistance,
                                             initialShadowPlaneOffset,
                                             initialShadowCascades,
                                             initialShadowCascade2Split,
                                             initialShadowCascade4Split);
	}
	...
	public static void ExportMapsGPU() {
		...
		if ((surforgeSettings != null) && (path != null) && (path != "") && (surforgeSettings.extentTexturePreview != null)) {
			...
			if (surforgeSettings.normalMap != null) {
				...
				if (export1.width != surforgeSettings.aoEdgesDirtDepth.width) export1.Reinitialize(surforgeSettings.aoEdgesDirtDepth.width, surforgeSettings.aoEdgesDirtDepth.height, TextureFormat.ARGB32, false);
				if (export2.width != surforgeSettings.aoEdgesDirtDepth.width) export2.Reinitialize(surforgeSettings.aoEdgesDirtDepth.width, surforgeSettings.aoEdgesDirtDepth.height, TextureFormat.ARGB32, false);
	...
	static void CreateMaterialForExportedMaps(string path, bool isLinearColorSpace) {
		...
		if (isLinearColorSpace) {
			...
			if (guidsAlbedo.Length > 0) {
                ...
                textureImporterAlbedo.sRGBTexture = false;
#else
...
#if UNITY_5_5_OR_NEWER
                textureImporterEmission.sRGBTexture = false;
#else
...
#if UNITY_5_5_OR_NEWER
                textureImporterHeightMap.sRGBTexture = false;
#else
...
#if UNITY_5_5_OR_NEWER
                textureImporterOcclusion.sRGBTexture = false;
#else
...
#if UNITY_5_5_OR_NEWER
                    textureImporterMetallic.sRGBTexture = false;
#else
...
#if UNITY_5_5_OR_NEWER
                    textureImporterSpecular.sRGBTexture = false;
#else
	...
	*/
	...
	public static void RebuildPolyLassoObject(GameObject originalTransformForCeltic, PolyLassoObject obj, float bevelAmount, int bevelSteps, float[] offsets, float[] heights,
                                              ...
                                              bool isChain,
                                              bool isScalePattern,
                                              float chainCrossAngle,
                                              Mesh[] scatterObjects,
                                              Vector3[] scatterObjectsScales,
                                              bool isShuffleScatterObjects,
                                              Vector3[] scatterRandomDegrees,
                                              Vector3[] scatterRandomOffsets,
                                              Vector3[] scatterRandomScales,
                                              Mesh scatterFirstObject,
                                              Mesh scatterLastObject,
                                              bool skipFirstScatter,
                                              bool skipLastScatter,
                                              float scatterTwistAngle,
                                              bool isScatterRandomizeAppearance,
                                              int scatterAppearRandomSeed,
                                              int scatterAppearRandomMin,
                                              int scatterAppearRandomMax,
                                              int scatterTRSRandomSeed) {
		...
		if (isTube) {
			...
			if (!noise) {
                ...
                if (isScatter && (!isAdaptive)) {
                    PreparePolyLassoScatterMesh(obj.gameObject);
                }
                else {
				...
				if (isAdaptive) {
                    ...
                    if (isScatter) {
                        Mesh[] scatterSnapshots = GetPolyLassoScatterSnapshots(scatterObjects);
                        Mesh firstScatterSnapshot = GetPolyLassoFirstScatterSnapshot(scatterFirstObject);
                        Mesh lastScatterSnapshot = GetPolyLassoLastScatterSnapshot(scatterLastObject);

                        MeshFilter sourceMeshFilter = (MeshFilter)obj.GetComponent<MeshFilter>();
                        Mesh sourceMesh = (Mesh)sourceMeshFilter.sharedMesh;

                        if (scatterSnapshots != null) {
                            List<Matrix4x4> scatterMatrices = null;
                            if (isChain) {
                                scatterMatrices = GetPolyLassoCurveChainScatterMatrices(pointsT, isOpen, scatterStepsX, scatterObjectsScales, 
                                                            chainCrossAngle, scatterRandomDegrees, heights);
                            }
                            else {
                                scatterMatrices = GetPolyLassoCurveScatterMatrices(pointsT, isOpen, scatterStepsX, scatterObjectsScales,
                                                            chainCrossAngle, scatterRandomDegrees, heights);
                            }
                            ...
                            int endIndex = scatterMatrices.Count;
                            if (skipLastScatter) {
                                endIndex = scatterMatrices.Count - 1;
                            }

                            for (int i = startIndex; i < endIndex; i++) {
                                if (i == startIndex) {
                                    if (firstScatterSnapshot != null) {
                                        InsertHEmeshSnapshot(firstScatterSnapshot, scatterMatrices[i], sourceMesh);
                                        continue;
                                    }
                                }
                                if (i == (endIndex - 1)) {
                                    if (lastScatterSnapshot != null) {
                                        InsertHEmeshSnapshot(lastScatterSnapshot, scatterMatrices[i], sourceMesh);
                                        continue;
                                    }
                                }
                                int snapshotIndex = GetPolyLassoScatterSnapshotIndex(i, scatterStepsX);
                                InsertHEmeshSnapshot(scatterSnapshots[snapshotIndex], scatterMatrices[i], sourceMesh);
                            }
                        }
                        else {
                            if (firstScatterSnapshot != null) {
                                Matrix4x4 m = GetPolyLassoCurveFirstScatterMatrix(pointsT, scatterObjectsScales, heights);
                                InsertHEmeshSnapshot(firstScatterSnapshot, m, sourceMesh);
                            }
                            if (lastScatterSnapshot != null) {
                                Matrix4x4 m = GetPolyLassoCurveLastScatterMatrix(pointsT, scatterObjectsScales, heights);
                                InsertHEmeshSnapshot(lastScatterSnapshot, m, sourceMesh);
                            }
                        }
			...
			if (!noise) {
				...
				if (isAdaptive) {
                    ...
                    if (isScatter) {
                        ...
                        Vector3[] pointsT = Points2DTo3D(vertsCleanBeveled, 0);
                        ...
                        if (scatterSnapshots != null) {
                            List<Matrix4x4> scatterMatrices = null;
                            scatterMatrices = GetPolyLassoShapeScatterMatrices(pointsT, scatterStepsX, scatterStepsZ, scatterObjectsScales,
                                                            isScalePattern, chainCrossAngle, scatterRandomDegrees, heights);

                            int startIndex = 0;
                            int endIndex = scatterMatrices.Count;

                            bool insertSnapshot = true;
                            int randomSeed = scatterAppearRandomSeed;
                            System.Random random = new System.Random(randomSeed);

                            for (int i = startIndex; i < endIndex; i++) {
                                if (isScatterRandomizeAppearance) {
                                    insertSnapshot = IsScatterAppear(random, scatterAppearRandomMin, scatterAppearRandomMax);
                                }
                                else {
                                    insertSnapshot = true;
                                }
        ...
        obj.scatterStepsX = scatterStepsX;
        obj.scatterStepsZ = scatterStepsZ;
        obj.isChain = isChain;
        obj.isScalePattern = isScalePattern;
        obj.chainCrossAngle = chainCrossAngle;
        obj.scatterObjects = scatterObjects;
        obj.scatterObjectsScales = scatterObjectsScales;
        obj.isShuffleScatterObjects = isShuffleScatterObjects;
        obj.scatterRandomDegrees = scatterRandomDegrees;
        obj.scatterRandomOffsets = scatterRandomOffsets;
        obj.scatterRandomScales = scatterRandomScales;
        obj.scatterFirstObject = scatterFirstObject;
        obj.scatterLastObject = scatterLastObject;
        obj.skipFirstScatter = skipFirstScatter;
        obj.skipLastScatter = skipLastScatter;
        obj.scatterTwistAngle = scatterTwistAngle;
        obj.isScatterRandomizeAppearance = isScatterRandomizeAppearance;
        obj.scatterAppearRandomSeed = scatterAppearRandomSeed;
        obj.scatterAppearRandomMin = scatterAppearRandomMin;
        obj.scatterAppearRandomMax = scatterAppearRandomMax;
        obj.scatterTRSRandomSeed = scatterTRSRandomSeed;
	...
	public static GameObject PolygonLassoBuildObject(GameObject originalTransformForCeltic, bool isChild, List<Vector3> points, float bevelAmount, int bevelSteps, float[] offsets, float[] heights,
		...
		RebuildPolyLassoObject(originalTransformForCeltic, polyLassoObject, bevelAmount, bevelSteps, offsets, heights,
                               ...
                               chainCrossAngle,
                               scatterObjects,
                               scatterObjectsScales,
                               isShuffleScatterObjects,
                               scatterRandomDegrees,
                               scatterRandomOffsets,
                               scatterRandomScales,
                               scatterFirstObject,
                               scatterLastObject,
                               skipFirstScatter,
                               skipLastScatter,
                               scatterTwistAngle,
                               isScatterRandomizeAppearance,
                               scatterAppearRandomSeed,
                               scatterAppearRandomMin,
                               scatterAppearRandomMax,
                               scatterTRSRandomSeed);
	...
	*/
	...
	static bool SplitPolyLassoObject(PolyLassoObject pObj, List<Vector3> splitLine) {
		...
		GameObject trim0 = PolygonLassoBuildObject(null, false, resultShapeA, pObj.bevelAmount, pObj.bevelSteps, pObj.offsets, pObj.heights,
                                                   ...
                                                   pObj.isChain,
                                                   pObj.isScalePattern,
                                                   pObj.chainCrossAngle,
                                                   pObj.scatterObjects,
                                                   pObj.scatterObjectsScales,
                                                   pObj.isShuffleScatterObjects,
                                                   pObj.scatterRandomDegrees,
                                                   pObj.scatterRandomOffsets,
                                                   pObj.scatterRandomScales,
                                                   pObj.scatterFirstObject,
                                                   pObj.scatterLastObject,
                                                   pObj.skipFirstScatter,
                                                   pObj.skipLastScatter,
                                                   pObj.scatterTwistAngle,
                                                   pObj.isScatterRandomizeAppearance,
                                                   pObj.scatterAppearRandomSeed,
                                                   pObj.scatterAppearRandomMin,
                                                   pObj.scatterAppearRandomMax,
                                                   pObj.scatterTRSRandomSeed);
		if (trim0 != null) {
			//set size
    ...
    */
	...
	static void BuildChildShape(GameObject originalTransformForCeltic, PolyLassoObject obj, List<Vector2> childShape, float childProfileVerticalOffset, int childProfileMatGroup, PolyLassoProfile profile) {
		...
		GameObject newObj = PolygonLassoBuildObject(originalTransformForCeltic, true, actualChildShape, profile.bevelAmount, profile.bevelSteps, profile.offsets, profile.heights,
		                                            	   ...
		                                            	   profile.usedForOverlapping, 
		                                           		   ...
		                                           		   profile.curveUVs,
                                                           profile.isScatter,
                                                           profile.scatterStepsX,
                                                           profile.scatterStepsZ,
                                                           profile.isChain,
                                                           profile.isScalePattern,
                                                           profile.chainCrossAngle,
                                                           profile.scatterObjects,
                                                           profile.scatterObjectsScales,
                                                           profile.isShuffleScatterObjects,
                                                           profile.scatterRandomDegrees,
                                                           profile.scatterRandomOffsets,
                                                           profile.scatterRandomScales,
                                                           profile.scatterFirstObject,
                                                           profile.scatterLastObject,
                                                           profile.skipFirstScatter,
                                                           profile.skipLastScatter,
                                                           profile.scatterTwistAngle,
                                                           profile.isScatterRandomizeAppearance,
                                                           profile.scatterAppearRandomSeed,
                                                           profile.scatterAppearRandomMin,
                                                           profile.scatterAppearRandomMax,
                                                           profile.scatterTRSRandomSeed);
	...
	static void OnUndoRedo() {
        ...
        #if UNITY_2023_1_OR_NEWER
        SurforgeSettings[] sceneSurforgeSettings = (SurforgeSettings[])GameObject.FindObjectsByType(typeof(SurforgeSettings), FindObjectsSortMode.InstanceID);
#else
...
#if UNITY_2023_1_OR_NEWER
        SurforgeRoot[] sceneSurforgeRootObjects = (SurforgeRoot[])GameObject.FindObjectsByType(typeof(SurforgeRoot), FindObjectsSortMode.InstanceID);
#else
...
#if UNITY_2023_1_OR_NEWER
        PolyLassoObject[] scenePolyLassoObjects = (PolyLassoObject[])GameObject.FindObjectsByType(typeof(PolyLassoObject), FindObjectsSortMode.InstanceID);
#else
...
#if UNITY_2023_1_OR_NEWER
        SurforgeTexturePreview[] sceneSurforgeTexturePreviewObjects = (SurforgeTexturePreview[])GameObject.FindObjectsByType(typeof(SurforgeTexturePreview), FindObjectsSortMode.InstanceID);
#else
	...
	*/
    ...
    private static Mesh[] GetPolyLassoScatterSnapshots(Mesh[] scatterObjects) {
        if (scatterObjects == null) return null;
        if (scatterObjects.Length == 0) return null;

        Mesh[] result = new Mesh[scatterObjects.Length];
        for (int i = 0; i < result.Length; i++) {
            result[i] = scatterObjects[i];
        }
        return result;
    }


    private static Mesh GetPolyLassoFirstScatterSnapshot(Mesh scatterFirstObject) {
        return scatterFirstObject;
    }

    private static Mesh GetPolyLassoLastScatterSnapshot(Mesh scatterLastObject) {
        return scatterLastObject;
    }


    private static List<Matrix4x4> GetPolyLassoCurveScatterMatrices(Vector3[] curve, bool isOpen, float[] scatterStepsX,
                                                      Vector3[] scatterObjectsScales, float chainCrossAngle, Vector3[] scatterRandomDegrees,
                                                      float[] heights) {
        List<Matrix4x4> result = new List<Matrix4x4>(16);

        int currentSegmentIndex = 0;
        int curveLength = curve.Length;
        int curveSegmentsToUse = curveLength;
        if (isOpen) {
            curveSegmentsToUse = curveLength - 1;
        }

        int objectsAdded = 0;
        Vector3 lastScatterPoint = curve[0];
        Vector3 dirToNextScatterPoint = Vector3.zero;
        float remainingDistance = 0;
        result.Add(GetPolyLassoCurveScatterMatrix(curve[0], curve[1], objectsAdded, scatterObjectsScales, chainCrossAngle,
                                                  scatterRandomDegrees, heights));
        objectsAdded++;
        while (currentSegmentIndex < curveSegmentsToUse) {
            int nextIndex = GetNextIndexCircular(currentSegmentIndex, 1, curveLength);

            float distanceToNextCurvePoint = Vector3.Distance(lastScatterPoint, curve[nextIndex]);
            float currentStepLength = GetPolyLassoCurveScatterStepLength(objectsAdded, scatterStepsX);

            if (distanceToNextCurvePoint >= (currentStepLength - remainingDistance)) {
                dirToNextScatterPoint = (curve[nextIndex] - curve[currentSegmentIndex]).normalized;
                Vector3 scatterPoint = lastScatterPoint + dirToNextScatterPoint * (currentStepLength - remainingDistance);
                result.Add(GetPolyLassoCurveScatterMatrix(scatterPoint, curve[nextIndex], objectsAdded, scatterObjectsScales, chainCrossAngle,
                                                  scatterRandomDegrees, heights));
                objectsAdded++;
                remainingDistance = 0;
                ...
                currentSegmentIndex++;
                remainingDistance = remainingDistance + distanceToNextCurvePoint;
                lastScatterPoint = curve[currentSegmentIndex];
            }
    ...
    private static List<Matrix4x4> GetPolyLassoCurveChainScatterMatrices(Vector3[] curve, bool isOpen, float[] scatterStepsX,
                                                              Vector3[] scatterObjectsScales, float chainCrossAngle, Vector3[] scatterRandomDegrees,
                                                              float[] heights) {
        List<Matrix4x4> result = new List<Matrix4x4>(16);
        ...
        int objectsAdded = 0;
        Vector3 scatterPoint = curve[0];
        Vector3 dirToNextScatterPoint = Vector3.zero;

        while (currentSegmentIndex < curveSegmentsToUse) {
            int nextIndex = GetNextIndexCircular(currentSegmentIndex, 1, curveLength);

            float distanceToNextCurvePoint = Vector3.Distance(scatterPoint, curve[nextIndex]);
            float currentStepLength = GetPolyLassoCurveScatterStepLength(objectsAdded, scatterStepsX);

            if (distanceToNextCurvePoint >= currentStepLength) {
                dirToNextScatterPoint = (curve[nextIndex] - scatterPoint).normalized;
                result.Add(GetPolyLassoCurveScatterMatrix(scatterPoint, curve[nextIndex], objectsAdded, scatterObjectsScales, chainCrossAngle,
                                                  scatterRandomDegrees, heights));
                objectsAdded++;
                scatterPoint = scatterPoint + dirToNextScatterPoint * currentStepLength;
            }
            else {
                Vector3 b = scatterPoint;
                Vector3 a = curve[nextIndex];
                int indexAfterNext = GetNextIndexCircular(currentSegmentIndex, 2, curveLength);
                float angle = Vector3.Angle((b - a).normalized, (curve[indexAfterNext] - a).normalized);
                float resultOffset = GetCurveScatterNextCurveSegmentOffset(b, a, currentStepLength, angle);

                if (resultOffset <= Vector3.Distance(curve[nextIndex], curve[indexAfterNext])) {
                    Vector3 rayDir = (curve[indexAfterNext] - curve[nextIndex]).normalized;
                    Vector3 offsettedPoint = curve[nextIndex] + rayDir * resultOffset;
                    dirToNextScatterPoint = (offsettedPoint - scatterPoint).normalized;
                    result.Add(GetPolyLassoCurveScatterMatrix(scatterPoint, offsettedPoint, objectsAdded, scatterObjectsScales, chainCrossAngle,
                                                  scatterRandomDegrees, heights));
                    objectsAdded++;
                    scatterPoint = scatterPoint + dirToNextScatterPoint * currentStepLength;
                }
    ...
    private static int GetNextIndexCircular(int currentIndex, int offset, int arrayLength) {
        int offsettedIndex = currentIndex + offset;
        if (offsettedIndex < arrayLength) return offsettedIndex;
        return offsettedIndex % arrayLength;
    }



    private static float GetPolyLassoCurveScatterStepLength(int stepIndex, float[] scatterStepsX) {
        if (scatterStepsX == null) return 1.0f;
        int stepsArrayLength = scatterStepsX.Length;
        if (stepsArrayLength == 0) return 1.0f;
        if (stepIndex < stepsArrayLength) return scatterStepsX[stepIndex];
        return (scatterStepsX[stepIndex % stepsArrayLength]);
    }


    private static int GetPolyLassoScatterSnapshotIndex(int stepIndex, float[] scatterStepsX) {
        if (scatterStepsX == null) return 0;
        int stepsArrayLength = scatterStepsX.Length;
        if (stepsArrayLength == 0) return 0;
        if (stepIndex < stepsArrayLength) return stepIndex;
        return stepIndex % stepsArrayLength;
    }


    private static Matrix4x4 GetPolyLassoCurveScatterMatrix(Vector3 scatterPoint, Vector3 nextCurvePoint, int stepIndex,
                                                      Vector3[] scatterObjectsScales, float chainCrossAngle, Vector3[] scatterRandomDegrees,
                                                      float[] heights) {
        Vector3 objectScale = Vector3.one;
        if (scatterObjectsScales != null) {
            int scalesArrayLength = scatterObjectsScales.Length;
            if (scalesArrayLength > 0) {
                if (stepIndex < scalesArrayLength) {
                    objectScale = scatterObjectsScales[stepIndex];
                }
                else {
                    objectScale = scatterObjectsScales[stepIndex % scalesArrayLength];
                }
            }
        }

        Quaternion objectRotation = Quaternion.identity;
        Vector3 nextPointDir = (nextCurvePoint - scatterPoint).normalized;
        objectRotation = Quaternion.LookRotation(nextPointDir, Vector3.up);
        if (chainCrossAngle != 0) {
            if (stepIndex % 2 == 0) {
                objectRotation *= Quaternion.Euler(Vector3.forward * chainCrossAngle);
            }
            else {
                objectRotation *= Quaternion.Euler(Vector3.forward * -chainCrossAngle);
            }
        }
        if (scatterRandomDegrees != null) {
            int randomDegreesLength = scatterRandomDegrees.Length;
            if (randomDegreesLength > 0) {
                int sign = Random.Range(0, 2) * 2 - 1; // returns 1 or -1
                Vector3 randomDegree = scatterRandomDegrees[Random.Range(0, randomDegreesLength)];
                objectRotation *= Quaternion.Euler(Vector3.right * randomDegree.x * sign);

                sign = Random.Range(0, 2) * 2 - 1;
                randomDegree = scatterRandomDegrees[Random.Range(0, randomDegreesLength)];
                objectRotation *= Quaternion.Euler(Vector3.up * randomDegree.y * sign);

                sign = Random.Range(0, 2) * 2 - 1;
                randomDegree = scatterRandomDegrees[Random.Range(0, randomDegreesLength)];
                objectRotation *= Quaternion.Euler(Vector3.forward * randomDegree.z * sign);
            }
        }
        float verticalOffset = 0;
        if (heights != null) {
            if (heights.Length > 0) {
                verticalOffset = heights[0];
            }
        }

        Matrix4x4 m = Matrix4x4.identity;
        m.SetTRS(new Vector3(scatterPoint.x, scatterPoint.y + verticalOffset, scatterPoint.z), objectRotation, objectScale);
        return m;
    }


    private static Matrix4x4 GetPolyLassoCurveFirstScatterMatrix(Vector3[] curve, Vector3[] scatterObjectsScales, float[] heights) {
        Matrix4x4 m = Matrix4x4.identity;
        Vector3 scatterPoint = curve[0];
        Vector3 nextPointDir = (curve[1] - scatterPoint).normalized;
        Quaternion objectRotation = Quaternion.LookRotation(nextPointDir, Vector3.up);

        Vector3 objectScale = Vector3.one;
        if (scatterObjectsScales != null) {
            int scalesArrayLength = scatterObjectsScales.Length;
            if (scalesArrayLength > 0) {
                objectScale = scatterObjectsScales[0];
            }
        }
        float verticalOffset = 0;
        if (heights != null) {
    ...
    private static Matrix4x4 GetPolyLassoCurveLastScatterMatrix(Vector3[] curve, Vector3[] scatterObjectsScales, float[] heights) {
        Matrix4x4 m = Matrix4x4.identity;
        int curveLength = curve.Length;
        Vector3 scatterPoint = curve[curveLength - 1];
        Vector3 nextPointDir = (curve[curveLength - 2] - scatterPoint).normalized;
        Quaternion objectRotation = Quaternion.LookRotation(nextPointDir, Vector3.up);
        ...
        if (scatterObjectsScales != null) {
            ...
            if (scalesArrayLength > 1) {
                objectScale = scatterObjectsScales[1];
            }
        }
        float verticalOffset = 0;
        if (heights != null) {
    ...
    private static float GetCurveScatterNextCurveSegmentOffset(Vector3 b, Vector3 a, float y, float angle) {
        bool isSharpAngle = false;
        if (angle < 90) {
            isSharpAngle = true;
        }

        float x = Vector3.Distance(b, a);
        float angleInRad = angle * Mathf.Deg2Rad;

        float z = 0;
        z = Mathf.Sin(angleInRad) * x;

        float u1 = Mathf.Sqrt(x * x - z * z);
        float u2 = Mathf.Sqrt(y * y - z * z);
    ...
    static void PreparePolyLassoScatterMesh(GameObject obj) {
    ...
    private static bool IsMeshLimitAllowsInsert(Mesh scatterSnapshot, Mesh sourceMesh) {
        int vertexLimit = 65000;

#if UNITY_2021_3_OR_NEWER
        vertexLimit = 10000000;
#endif

        if ((sourceMesh.vertices.Length + scatterSnapshot.vertices.Length) <= vertexLimit) {
            return true;
        }
        return false;
    }
    ...
    private static void InsertHEmeshSnapshot(Mesh scatterSnapshot, Matrix4x4 transformMatrix, Mesh sourceMesh) {
        if (IsMeshLimitAllowsInsert(scatterSnapshot, sourceMesh)) {
            AddMeshSnapshotToTheMesh(scatterSnapshot, transformMatrix, sourceMesh);
        }
    }


    private static void AddMeshSnapshotToTheMesh(Mesh scatterSnapshot, Matrix4x4 transformMatrix, Mesh sourceMesh) {
        Vector3[] sourceVertices = sourceMesh.vertices;
        int sourceVerticesLength = sourceVertices.Length;
        int[] sourceTriangles = sourceMesh.triangles;
        int sourceTrianglesLength = sourceTriangles.Length;

        Vector3[] snapshotVertices = scatterSnapshot.vertices;
        int snapshotVerticesLength = snapshotVertices.Length;
        int[] snapshotTriangles = scatterSnapshot.triangles;
        int snapshotTrianglesLength = snapshotTriangles.Length;

        Vector3[] vertices = new Vector3[sourceVerticesLength + snapshotVerticesLength];
        for (int v = 0; v < sourceVerticesLength; v++) {
            vertices[v] = sourceVertices[v];
        }
        for (int w = 0; w < snapshotVerticesLength; w++) {
            vertices[sourceVerticesLength + w] = transformMatrix.MultiplyPoint3x4(snapshotVertices[w]);
        }


        int[] triangles = new int[sourceTrianglesLength + snapshotTrianglesLength];
        for (int t = 0; t < sourceTrianglesLength; t++) {
            triangles[t] = sourceTriangles[t];
        }

        for (int q = 0; q < snapshotTrianglesLength; q++) {
            triangles[sourceTrianglesLength + q] = snapshotTriangles[q] + sourceVerticesLength;
        }
    ...
    private static List<Matrix4x4> GetPolyLassoShapeScatterMatrices(Vector3[] shape, float[] scatterStepsX, float[] scatterStepsZ,
                                                      Vector3[] scatterObjectsScales, bool isScalePattern, float chainCrossAngle, 
                                                      Vector3[] scatterRandomDegrees, float[] heights) {
        List<Matrix4x4> result = new List<Matrix4x4>(128);
        ...
        Vector3 polyLassoPointsCenter = new Vector3(midX, 0, midZ);
        bool crossAngleReversed = false;

        Vector3 startPos = new Vector3(polyLassoPointsCenter.x, polyLassoPointsCenter.y, polyLassoPointsCenter.z);
        AddPolyLassoShapeScatterRow(shape, scatterStepsX, scatterObjectsScales, chainCrossAngle, scatterRandomDegrees, 
                                    heights, startPos, result, minX, maxX, 512, crossAngleReversed);

        float scalePatternOffset = 0;
        if (isScalePattern) {
            scalePatternOffset = GetPolyLassoScalePatternStep(scatterStepsX);
        }

        for (int i = 0; i < 512; i++) {
            float stepZ = GetPolyLassoShapeScatterStepZ(0, scatterStepsZ);
            if (i % 2 == 0) {
                crossAngleReversed = true;
                startPos = new Vector3(startPos.x + scalePatternOffset, startPos.y, startPos.z + stepZ);
            }
            else {
                crossAngleReversed = false;
                startPos = new Vector3(startPos.x - scalePatternOffset, startPos.y, startPos.z + stepZ);
            }
            if (startPos.z > maxZ) break;
            AddPolyLassoShapeScatterRow(shape, scatterStepsX, scatterObjectsScales, chainCrossAngle, scatterRandomDegrees,
                                        heights, startPos, result, minX, maxX, 512, crossAngleReversed);
        }

        startPos = new Vector3(polyLassoPointsCenter.x, polyLassoPointsCenter.y, polyLassoPointsCenter.z);
        for (int i = 0; i < 512; i++) {
            float stepZ = GetPolyLassoShapeScatterStepZ(0, scatterStepsZ);
            if (i % 2 == 0) {
                crossAngleReversed = true;
    ...
    private static float GetPolyLassoScalePatternStep(float[] scatterStepsX) {
        if (scatterStepsX == null) return 0.5f;
        if (scatterStepsX.Length == 0) return 0.5f;
        return scatterStepsX[0] / 2.0f;
    }


    private static void AddPolyLassoShapeScatterRow(Vector3[] shape, float[] scatterStepsX, Vector3[] scatterObjectsScales, 
                                                     float chainCrossAngle, Vector3[] scatterRandomDegrees, float[] heights,
                                                     Vector3 startPos, List<Matrix4x4> result,
                                                     float minX, float maxX, int attempts, bool crossAngleReversed) {
        Matrix4x4 m = Matrix4x4.identity;
        Vector3 scatterPos = Vector3.zero;
        int stepIndex = 0;

        scatterPos = new Vector3(startPos.x, startPos.y, startPos.z);
        if (IsPointInPolygon(shape.Length, shape, scatterPos.x, scatterPos.z, true)) {
            m = GetPolyLassoShapeScatterMatrix(scatterPos, stepIndex, scatterObjectsScales, chainCrossAngle, scatterRandomDegrees, 
                                               heights, crossAngleReversed);
            result.Add(m);
        }

        for (int i = 0; i < attempts; i++) {
            float stepX = GetPolyLassoShapeScatterStepX(stepIndex, scatterStepsX) * 0.5f +
                          GetPolyLassoShapeScatterStepX(stepIndex + 1, scatterStepsX) * 0.5f;
            stepIndex++;
            scatterPos = new Vector3(scatterPos.x + stepX, scatterPos.y, scatterPos.z);
            if (scatterPos.x > maxX) break;
            if (IsPointInPolygon(shape.Length, shape, scatterPos.x, scatterPos.z, true)) {
                m = GetPolyLassoShapeScatterMatrix(scatterPos, stepIndex, scatterObjectsScales, chainCrossAngle, scatterRandomDegrees,
                                                   heights, crossAngleReversed);
                result.Add(m);
        ...
        scatterPos = new Vector3(startPos.x, startPos.y, startPos.z);
        stepIndex = scatterStepsX.Length;
        for (int i = 0; i < attempts; i++) {
            stepIndex--;
            if (stepIndex < 0) {
                stepIndex = scatterStepsX.Length - 1;
            }

            float stepX = GetPolyLassoShapeScatterStepX(stepIndex, scatterStepsX) * 0.5f +
                          GetPolyLassoShapeScatterStepX(stepIndex + 1, scatterStepsX) * 0.5f;
            scatterPos = new Vector3(scatterPos.x - stepX, scatterPos.y, scatterPos.z);
            if (scatterPos.x < minX) break;
            if (IsPointInPolygon(shape.Length, shape, scatterPos.x, scatterPos.z, true)) {
                m = GetPolyLassoShapeScatterMatrix(scatterPos, stepIndex, scatterObjectsScales, chainCrossAngle, scatterRandomDegrees,
                                                   heights, crossAngleReversed);
                result.Add(m);
    ...
    private static float GetPolyLassoShapeScatterStepX(int stepIndex, float[] scatterStepsX) {
        if (scatterStepsX == null) return 1.0f;
        int stepsArrayLength = scatterStepsX.Length;
        if (stepsArrayLength == 0) return 1.0f;
        if (stepIndex < stepsArrayLength) return scatterStepsX[stepIndex];
    ...
    private static float GetPolyLassoShapeScatterStepZ(int stepIndex, float[] scatterStepsZ) {
        if (scatterStepsZ == null) return 1.0f;
        int stepsArrayLength = scatterStepsZ.Length;
        if (stepsArrayLength == 0) return 1.0f;
        if (stepIndex < stepsArrayLength) return scatterStepsZ[stepIndex];
        return (scatterStepsZ[stepIndex % stepsArrayLength]);
    }


    private static bool IsPointInPolygon(int polyCorners, Vector3[] poly, float x, float z, bool isOutside) {
        int i, j = polyCorners - 1;
        bool oddNodes = false;

        for (i = 0; i < polyCorners; i++) {
            if ((poly[i].z < z && poly[j].z >= z
                 || poly[j].z < z && poly[i].z >= z)
                && (poly[i].x <= x || poly[j].x <= x)) {
                oddNodes ^= (poly[i].x + (z - poly[i].z) / (poly[j].z - poly[i].z) * (poly[j].x - poly[i].x) < x);
            }
            j = i;
        }
        if (isOutside) return oddNodes;
    ...
    private static Matrix4x4 GetPolyLassoShapeScatterMatrix(Vector3 scatterPoint, int stepIndex, Vector3[] scatterObjectsScales, 
                                                             float chainCrossAngle, Vector3[] scatterRandomDegrees, float[] heights,
                                                             bool crossAngleReversed) {
        Vector3 objectScale = Vector3.one;
        if (scatterObjectsScales != null) {
            int scalesArrayLength = scatterObjectsScales.Length;
            if (scalesArrayLength > 0) {
    ...
    private static bool IsScatterAppear(System.Random random, int scatterAppearRandomMin, int scatterAppearRandomMax) {
        int randomNum = random.Next(0, 100);
        if ((randomNum >= scatterAppearRandomMin) && (randomNum < scatterAppearRandomMax)) return true;
        return false;
    }

...
public class GPURender : MonoBehaviour {
    ...
    public GameObject aoLightsPrefabHigh;
    public GameObject aoLightsPrefabDx11High;
    public GameObject aoLightsPrefabLinearHigh;
    public GameObject aoLightsPrefabDx11LinearHigh;
    ...
    void InstantiateAOLights() {
        if (SystemInfo.graphicsDeviceType == UnityEngine.Rendering.GraphicsDeviceType.Direct3D11) {
            if (PlayerSettings.colorSpace == ColorSpace.Linear) {
                if (highQualityAo) aoLights = (GameObject)Instantiate(aoLightsPrefabDx11LinearHigh);
                else aoLights = (GameObject)Instantiate(aoLightsPrefabDx11Linear);
            }
            else {
                if (highQualityAo) aoLights = (GameObject)Instantiate(aoLightsPrefabDx11High);
                else aoLights = (GameObject)Instantiate(aoLightsPrefabDx11);
            }
        }
        else {
            if (PlayerSettings.colorSpace == ColorSpace.Linear) {
                if (highQualityAo) aoLights = (GameObject)Instantiate(aoLightsPrefabLinearHigh);
                else aoLights = (GameObject)Instantiate(aoLightsPrefabLinear);
            }
            else {
                if (highQualityAo) aoLights = (GameObject)Instantiate(aoLightsPrefabHigh);
                else aoLights = (GameObject)Instantiate(aoLightsPrefab);
            }
        }
    }
    ...
    ShadowResolution initialShadowResolution;
    ShadowProjection initialShadowProjection;
    float initialShadowDistance;
    float initialShadowPlaneOffset;
    int initialShadowCascades;
    float initialShadowCascade2Split;
    Vector3 initialShadowCascade4Split;

    public void StoreProjectShadowSettings(ShadowResolution shadowResolution,
        ...
        initialShadowResolution = shadowResolution;
        initialShadowProjection = shadowProjection;
        initialShadowDistance = shadowDistance;
        initialShadowPlaneOffset = shadowPlaneOffset;
        initialShadowCascades = shadowCascades;
        initialShadowCascade2Split = shadowCascade2Split;
        initialShadowCascade4Split = shadowCascade4Split;
    }

    void RevertProjectShadowSettings() {
        QualitySettings.shadowResolution = initialShadowResolution;
        QualitySettings.shadowProjection = initialShadowProjection;
        QualitySettings.shadowDistance = initialShadowDistance;
        QualitySettings.shadowNearPlaneOffset = initialShadowPlaneOffset;
        QualitySettings.shadowCascades = initialShadowCascades;
        QualitySettings.shadowCascade2Split = initialShadowCascade2Split;
        QualitySettings.shadowCascade4Split = initialShadowCascade4Split;
    }
    ...
    */
    ...
    public void SetAoMode(int mode) {
        ...
        if (mode == 0) highQualityAo = false;
        else highQualityAo = true;
    }

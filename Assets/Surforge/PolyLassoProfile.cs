...
public class PolyLassoProfile : MonoBehaviour {
    ...
    public bool isScatter; //if isTube perform Curve Scatter, otherwise Shape scatter
    public float[] scatterStepsX; //for Curve Scatter and Shape fill Scatter
    public float[] scatterStepsZ; //for Shape fill Scatter only
    public bool isChain; //rotate objects towards next and not instantiate last one
    public bool isScalePattern; //for Shape fill Scatter, every next row offsetted for half of scatterStepX to form scale pattern
    public float chainCrossAngle; //the angle chain segment rotated one way and the other in course anlong towards vector
                                   //for shape scatter this forms chainmail pattern, tilting objects of each row in opposite direction
    public Mesh[] scatterObjects; //Mesh array for scatter
    public Vector3[] scatterObjectsScales; //scales of scatter objects
    public bool isShuffleScatterObjects;
    public Vector3[] scatterRandomDegrees; //rotation amounts added to each axis, picked at random
    public Vector3[] scatterRandomOffsets; //position offsets added to each axis, picked at random
    public Vector3[] scatterRandomScales; //scale amounts multiplied to each axis, picked at random

    public Mesh scatterFirstObject; //replaces first scatter objects if present. If scatterObjects null or empty added at start of curve
    public Mesh scatterLastObject; //replaces last scatter objects if present. If scatterObjects null or empty added at end of curve
    public bool skipFirstScatter;
    public bool skipLastScatter;
    public float scatterTwistAngle; //additional twist rotation of each next scattered object in degrees

    public bool isScatterRandomizeAppearance; //is enabled, not every scattered object added to the mesh
    public int scatterAppearRandomSeed; //system random seed for is scattered object added or not to the mesh
    public int scatterAppearRandomMin; //the minimum of 0-100 random for scattered object to appear
    public int scatterAppearRandomMax;

    public int scatterTRSRandomSeed; //system random seed for scatterRandomDegrees, Offsets and Scales

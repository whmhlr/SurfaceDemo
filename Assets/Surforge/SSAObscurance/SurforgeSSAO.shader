...
#else
#define NUM_SAMPLES (110)
#endif


#define FAR_PLANE_Z (300)
#define NUM_SPIRAL_TURNS (70)
#define bias (0.1) 
...
#define R                   (4)
	...
	float2 TapLocation(int sampleNumber, float spinAngle, out float ssR) {
		...
		float alpha = float(sampleNumber + 0.5) * (10.0 / (NUM_SAMPLES * _samplesMult));
	...
	float SampleAO(in float2 ssC, in float3 C, in float3 n_C, in float ssDiskRadius, in int tapIndex, in float randomPatternRotationAngle)
	{
		...
		float ssR;
		//float2 unitOffset = TapLocation(tapIndex, randomPatternRotationAngle, ssR) * 500;
	...
	float4 fragAO(v2f i) : COLOR
	{
		...
		float3 n_C = normalValues;
		n_C = float3(-n_C.x, -n_C.y, -n_C.z);
		...
		float sum = 0.0;
		#ifdef SHADER_API_D3D11
		for (int l = 0; l < (NUM_SAMPLES * _samplesMult); ++l) {
		...
		float A = max(0.0, 1.0 - sum * _Intensity * (5.0 / (NUM_SAMPLES * _samplesMult)));
		...
		float4 fragApplySoft(v2f i) : COLOR
	{
		...
		float ao = tex2D(_AOTex, i.uv.xy).r;
		ao += tex2D(_AOTex, i.uv.xy + _MainTex_TexelSize.xy * 0.75).r;
		ao += tex2D(_AOTex, i.uv.xy - _MainTex_TexelSize.xy * 0.75).r;
		ao += tex2D(_AOTex, i.uv.xy + _MainTex_TexelSize.xy * float2(-0.75,0.75)).r;
		ao += tex2D(_AOTex, i.uv.xy - _MainTex_TexelSize.xy * float2(-0.75,0.75)).r;
